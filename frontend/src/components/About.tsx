...
import { Code2, Brain, Cpu, Timer, FileText, Clock, PenTool, Workflow } from 'lucide-react';
...
const About: React.FC = () => {
  const features = [
    {
      ...
      title: "Input Support",
      description: "Upload handwritten notes or scanned snippets as JPEG or PNG images, up to 5MB"
    },
    {
      icon: <Brain className="w-6 h-6" />,
      title: "AI-Powered OCR",
      description: "A CNN trained on handwriting samples runs entirely client-side via TensorFlow.js"
    },
    {
      icon: <PenTool className="w-6 h-6" />,
      title: "Per-Character Confidence",
      description: "Each recognized character is individually scored and color-coded, not just one aggregate number"
    },
    {
      icon: <Clock className="w-6 h-6" />,
      title: "Local History",
      description: "Past recognitions are saved to your browser's localStorage so you can revisit them later"
    }
  ];
  ...
  const techStack = [
    ...
    },
    {
      ...
      title: "Inference",
      description: "TensorFlow.js runs the CNN entirely client-side — no image is ever uploaded to a server"
    },
    {
      icon: <Workflow className="w-6 h-6" />,
      title: "Training",
      description: "Python + TensorFlow/Keras (see /training) build the CNN and convert it to TensorFlow.js format"
    },
    {
      icon: <Timer className="w-6 h-6" />,
      title: "Processing",
      description: "Real-time feedback with per-character confidence scores and instant image preview"
    }
  ];
          ...
          <div className="space-y-4">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-[var(--text-brown)] opacity-80">Initialize the React + TypeScript + Vite app and the Python training pipeline</p>
              </div>
            </div>
            <div className="flex items-start gap-4">
